...
>>>>>>> 3d2cb3530d7d4936d9b18a779964453f85103e76
const checkDelete = () => {
...
router.get('/42', (req, res) => {
    ...
    const fetchSessionInfo = async (id) => {
        try {
            const sql = `SELECT users.id, status, username, firstname, lastname, path
                         FROM users
                                    ...
                                    if (bool === false) {
                                        generateRandomUsername()
                                            .then((randomUsername) => {
                                                ...
                                                var sql = `INSERT INTO users (username, firstname, lastname, email,
                                                                              password, verifycode, fortytwo_id, status)
                                        ...
                                        var sql = `SELECT users.id, status, username, firstname, lastname, path
                                                   FROM users
                                    ...
                                    if (bool === false) {
                                        generateRandomUsername()
                                            .then((randomUsername) => {
                                                const verifycode = crypto.createHash(algo).update(randomUsername).digest('base64url');
                                                var sql = `INSERT INTO users (username, firstname, lastname, email,
                                                                              password, verifycode, github_id, status)
                                                           VALUES ($1, $2, $3, $4, $5, $6, $7, $8);`
